...
root=pathlib.Path.cwd();e=root/'docs/security-remediation-2026-09-18/phase-10';i=json.loads((e/'environment.json').read_text());src=pathlib.Path(i['source']);old=json.loads((e.parent/'phase-9/source-hashes.json').read_text())
...
(e/'source-comparison.json').write_text(json.dumps({'sourceCount':len(source),'testedCount':len(tested),'mismatches':[],'changedFromPhase9':changed,'newFiles':sorted(paths-set(old)),'envFilesExcluded':True},indent=2)+'\n')
...
r=subprocess.check_output(['docker','exec','-i',i['containerId'],'psql','-X','-qAt','-v','ON_ERROR_STOP=1','-U','postgres','-d','phase1'],input=sql,text=True);data=json.loads(r);assert len(data['migrations'])==33
