...
root=pathlib.Path.cwd(); evidence=root/'docs/security-remediation-2026-09-18/phase-7'; run=pathlib.Path(tempfile.mkdtemp(prefix='sana-phase7-')); src=run/'source'; src.mkdir()
...
name='sana-phase7-'+run.name.split('-')[-1]; label='sana.task='+name
cid=subprocess.check_output(['docker','run','-d','--name',name,'--label',label,'-e','POSTGRES_PASSWORD=phase7_fixture','-e','POSTGRES_DB=phase1','-p','127.0.0.1::5432','postgres:16'],text=True).strip()
port=subprocess.check_output(['docker','port',cid,'5432/tcp'],text=True).strip().split(':')[-1]
env={k:os.environ[k] for k in ['PATH','HOME','TMPDIR'] if k in os.environ}
env.update(DATABASE_URL=f'postgresql://phase1_runtime:phase7_runtime@127.0.0.1:{port}/phase1',PHASE1_OWNER_URL=f'postgresql://postgres:phase7_fixture@127.0.0.1:{port}/phase1',PHASE1_ISOLATED_DB='true',ZARINPAL_SANDBOX='true',ZARINPAL_MERCHANT_ID='',SMS_API_KEY='',SESSION_SECRET='phase7-isolated-synthetic-secret-20260919',OTP_VERIFY_LIMIT_SECRET='phase7-only-protected-limiter-secret-20260919',OTP_VERIFY_TRUST_PROXY='false',GHASEDAK_API_KEY='',SMS_SEND_IN_DEVELOPMENT='false',APP_URL='http://127.0.0.1:3293',UPLOAD_DIR=str(run/'uploads'),GIT_DIR=str(root/'.git'),GIT_WORK_TREE=str(root),NEXT_TELEMETRY_DISABLED='1')
...
with (evidence/'setup.log').open('w') as log:
 ...
 subprocess.run(['docker','exec',cid,'psql','-U','postgres','-d','phase1','-c',"CREATE ROLE phase1_runtime LOGIN PASSWORD 'phase7_runtime' NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT"],stdout=log,stderr=subprocess.STDOUT,check=True)
