...
root=pathlib.Path.cwd();e=root/'docs/security-remediation-2026-09-18/phase-12';i=json.loads((e/'environment.json').read_text());src=pathlib.Path(i['source']);old=json.loads((e.parent/'phase-11/source-hashes.json').read_text())
...
(e/'source-comparison.json').write_text(json.dumps({'sourceCount':len(source),'testedCount':len(tested),'mismatches':[],'changedFromPhase11':changed,'newFiles':sorted(paths-set(old)),'envFilesExcluded':True},indent=2)+'\n')
