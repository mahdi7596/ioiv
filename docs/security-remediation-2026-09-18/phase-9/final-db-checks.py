...
root=pathlib.Path('/Users/mahdi/Documents/work/ioiv');e=root/'docs/security-remediation-2026-09-18/phase-9';i=json.loads((e/'environment.json').read_text());cid=i['containerId'];env=json.loads((pathlib.Path(i['run'])/'env.json').read_text());src=pathlib.Path(i['source'])
...
DO $$ BEGIN
 ...
 PERFORM count(*) FROM "LegacyFileBindingRepair";
 IF has_table_privilege(current_user,'"ApplicationFile"','UPDATE') OR has_table_privilege(current_user,'"ApplicationFile"','DELETE') THEN RAISE EXCEPTION 'legacy metadata mutation allowed'; END IF;
 IF has_table_privilege(current_user,'"LegacyFileBindingRepair"','UPDATE') OR has_table_privilege(current_user,'"LegacyFileBindingRepair"','DELETE') THEN RAISE EXCEPTION 'legacy repair mutation allowed'; END IF;
 IF has_table_privilege(current_user,'"LegacyUploadCandidate"','DELETE') OR has_table_privilege(current_user,'"LegacyFileBinding"','TRUNCATE') OR has_table_privilege(current_user,'"LegacyFileDeletionIntent"','DELETE') THEN RAISE EXCEPTION 'legacy evidence erasure allowed'; END IF;
...
sql('CREATE DATABASE phase9_m9','m9-create-db.log','postgres');m9env={**env,'DATABASE_URL':urllib.parse.urlunsplit(urllib.parse.urlsplit(env['PHASE1_OWNER_URL'])._replace(path='/phase9_m9'))}
...
sql((root/'prisma/tests/facilities-m9-supplier-provisioning.integration.sql').read_text(),'m9-assertions-final.log','phase9_m9');results['m9']='PASS dry-run, apply twice, exactly four suppliers and no programme/intake/template/user/admin'
