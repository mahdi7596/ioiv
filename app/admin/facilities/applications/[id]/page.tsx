...
export default async function FacilitiesReviewDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AppShell area="admin" eyebrow="جزئیات تسهیلات" title={application.companySnapshot?.name || "پرونده تسهیلات"} description="اطلاعات، مدارک خصوصی، پرداخت و چرخه‌های اصلاح پرونده." action={<Link className="button button--ghost button--back" href="/admin/facilities/applications"><ArrowRight aria-hidden="true" size={19} />بازگشت</Link>}>
