...
export default async function FacilitiesConfigurationPage() {
  ...
  return <AppShell area="admin" eyebrow="تسهیلات" title="پیکربندی برنامه تسهیلات" description="فقط مدیر ارشد فعال به این تنظیمات خصوصی دسترسی دارد." action={<Link className="button button--ghost button--back" href="/admin" aria-label="بازگشت" title="بازگشت"><ArrowRight aria-hidden="true" size={19} strokeWidth={2} />بازگشت</Link>}><FacilitiesConfiguration initial={{ programmeEnabled: data.programme?.isEnabled ?? false, suppliers: data.suppliers.map((supplier) => ({ ...supplier, templates: supplier.templates.map((template) => ({ ...template, publishedAt: template.publishedAt?.toISOString() ?? null })) })), intakes: data.intakes.map((intake) => ({ ...intake, maximumAmountRial: intake.maximumAmountRial.toString() })) }} /></AppShell>;
}
