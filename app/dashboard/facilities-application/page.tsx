...
export default async function FacilitiesApplicationPage({ searchParams }: { searchParams: Promise<{ payment?: string; submitted?: string }> }) {
  ...
  return <AppShell area="user" eyebrow="تسهیلات" title="درخواست تسهیلات" description="پیش‌نویس را کامل و بررسی کنید؛ پرداخت و ارسال پس از بررسی نهایی انجام می‌شود." action={<Link href="/dashboard" className="button button--ghost button--back" aria-label="بازگشت به داشبورد" title="بازگشت به داشبورد"><ArrowRight aria-hidden="true" size={19} strokeWidth={2} />بازگشت</Link>}><FacilitiesApplicationWizard data={JSON.parse(JSON.stringify(data))} notice={params.payment || params.submitted} /></AppShell>;
}
