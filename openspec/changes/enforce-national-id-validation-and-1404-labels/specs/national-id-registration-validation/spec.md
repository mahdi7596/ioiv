...
The system SHALL accept `شناسه ملی شرکت` only when the normalized value contains exactly 11 digits.

#### Scenario: Ten digit national ID is rejected inline
...
- **WHEN** a registering user enters a `شناسه ملی شرکت` containing fewer than 11 normalized digits
- **THEN** the frontend shows an inline error beside the registration form field
- **AND** the user cannot complete registration with that value
...
#### Scenario: Existing application national ID is rejected
- **WHEN** a new user completes OTP verification with a valid national ID that already exists in application records
- **THEN** the system does not create a new user
- **AND** the frontend shows a Persian error explaining that the national ID has already been registered
