...
vi.mock("@/lib/actions/facilities-review", () => ({
  performFacilitiesReview: vi.fn(), startFacilitiesReview: vi.fn(), requestFacilitiesCorrection: vi.fn(), completeFacilitiesValidation: vi.fn(), retryFacilitiesCorrectionSms: vi.fn(),
}));
...
describe("facilities review action states", () => {
  it("shows only start review for a submitted application", () => {
    const markup = renderToStaticMarkup(<FacilitiesReviewActions reviewVersion={0} applicationId="app_1" status="SUBMITTED" />);
  ...
  it("shows correction and completion controls while under review", () => {
    const markup = renderToStaticMarkup(<FacilitiesReviewActions reviewVersion={0} applicationId="app_1" status="UNDER_REVIEW" />);
  ...
  it("shows SMS retry without reopening a correction", () => {
    const markup = renderToStaticMarkup(<FacilitiesReviewActions reviewVersion={0} applicationId="app_1" status="NEEDS_EDIT" failedCorrectionId="correction_1" />);
